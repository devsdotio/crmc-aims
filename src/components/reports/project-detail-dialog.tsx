...
export function ProjectDetailDialog({
  ...
  const [cachedProject, setCachedProject] = useState<ProjectReportRow | null>(project);
  ...
  const costBreakdownData = [
    ...
    { name: "Direct Ops & Services", value: otherCost > 0 ? otherCost : totalCost > 0 ? 0 : 1 },
  ].filter((item) => item.value > 0);

  const resourceData = [
    { name: "Capital Assets", count: effectiveProject.assignedAssetsCount },
    { name: "Consumable Units", count: effectiveProject.consumablesConsumedCount },
  ];
    ...
    <>
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-border/80 bg-card px-5 py-2 overflow-x-auto text-xs gap-3">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button
              type="button"
              onClick={() =>
                document.getElementById("sec-project-overview")?.scrollIntoView({ behavior: "smooth" })
              }
            ...
            <button
              type="button"
              onClick={() =>
                document.getElementById("sec-project-assets")?.scrollIntoView({ behavior: "smooth" })
              }
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-text-secondary hover:text-text hover:bg-bg-subtle transition-colors cursor-pointer"
            >
              <Package className="h-3.5 w-3.5 text-blue-600" />
              <span>Assigned Assets ({effectiveProject.assignedAssetsCount})</span>
            </button>
            <button
              type="button"
              onClick={() =>
                document.getElementById("sec-project-supplies")?.scrollIntoView({ behavior: "smooth" })
              }
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-text-secondary hover:text-text hover:bg-bg-subtle transition-colors cursor-pointer"
            >
              <Layers className="h-3.5 w-3.5 text-indigo-600" />
              <span>Consumables ({effectiveProject.consumablesConsumedCount})</span>
            </button>
            <button
              type="button"
              onClick={() =>
                document.getElementById("sec-project-financials")?.scrollIntoView({ behavior: "smooth" })
              }
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-text-secondary hover:text-text hover:bg-bg-subtle transition-colors cursor-pointer"
            >
          ...
          <section id="sec-project-overview" className="space-y-5">
            ...
            <StatCardGrid className="grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <StatCard
                ...
                subtitle={canViewCosts ? "All tracked spend" : "Admin view only"}
                icon={DollarSign}
                tone="accent"
                toneValue
              />
                  ...
                  <div>
                    <div className="flex items-center justify-between border-b border-border/60 pb-2.5 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h4 className="text-xs font-bold text-text">Cost Breakdown</h4>
                  ...
                  <div>
                    <div className="flex items-center justify-between border-b border-border/60 pb-2.5 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h4 className="text-xs font-bold text-text">Resource Deployment</h4>
                          <p className="text-[10px] text-text-secondary">
                    ...
                    <div className="h-48 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Tooltip
                            cursor={false}
                            formatter={(val: unknown) => [Number(val || 0).toLocaleString(), "Units"]}
                            contentStyle={{
                              backgroundColor: "var(--card, #ffffff)",
                              borderRadius: "10px",
                              border: "1px solid var(--border, #E5E7EB)",
                ...
                <div className="rounded-2xl border border-border/80 bg-card p-4 shadow-2xs space-y-3 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3.5 text-xs pt-3">
                      <div>
                        <span className="text-text-secondary block text-[11px] mb-0.5">Department</span>
                      ...
                      <div>
                        <span className="text-text-secondary block text-[11px] mb-0.5">Project Lead</span>
                        <span className="font-semibold text-text flex items-center gap-1.5">
                      ...
                      <div className="col-span-2">
                        <span className="text-text-secondary block text-[11px] mb-0.5">Execution Schedule</span>
                        <span className="font-mono text-text flex items-center gap-1.5">
                          <Calendar className="h-3.5 w-3.5 text-text-secondary" />
                          {effectiveProject.startDate} → {effectiveProject.endDate || "Ongoing"}
                        </span>
                      </div>
                    </div>
          ...
          <section id="sec-project-assets" className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-text">Assigned Capital Equipment</h3>
                    <p className="text-xs text-text-secondary">
                      Itemized list of physical assets actively deployed to {effectiveProject.projectCode}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {assignedAssets.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-border">
                ...
                <div className="text-sm font-bold text-text">No Assets Currently Assigned</div>
                  ...
                  <table className="w-full text-left text-xs">
                    <thead className="border-b border-border/80 bg-bg-subtle/80 text-[10px] font-bold uppercase tracking-wider text-text-secondary">
                      <tr>
                        ...
                        {canViewCosts && <th className="px-4 py-3 text-right">Book Value</th>}
                      </tr>
                    </thead>
                      ...
                      {filteredAssets.map((asset) => (
                        <tr key={asset.id} className="hover:bg-bg-subtle/50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            ...
                            {asset.location && (
                              <div className="text-[10px] text-text-secondary">{asset.location}</div>
                            )}
                          </td>
                          <td className="px-4 py-3">
          ...
          <section id="sec-project-supplies" className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-text">Consumable Stocks Ledger</h3>
                    <p className="text-xs text-text-secondary">
                      Itemized warehouse materials and supplies issued to {effectiveProject.projectCode}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {canViewCosts && effectiveProject.consumablesValue != null && (
            ...
            {consumedSupplies.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-border">
                ...
                <div className="text-sm font-bold text-text">No Consumables Consumed</div>
                <p className="text-xs text-text-secondary max-w-sm mt-1">
                  No warehouse materials or supplies have been issued to this project.
                </p>
              </div>
          ...
          <section id="sec-project-financials" className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-text">Financial Audit &amp; Cost Ledger</h3>
                  <p className="text-xs text-text-secondary">
                    Expenditure breakdown and audited cost totals for {effectiveProject.projectName}
                  </p>
                </div>
              </div>
            </div>
            ...
            {canViewCosts ? (
              <div className="rounded-2xl border border-border/80 bg-card p-5 space-y-4 shadow-2xs">
                <div className="flex items-center justify-between border-b border-border/60 pb-3">
                  <div>
                    <div className="text-xs text-text-secondary">Total Project Expenditure</div>
                    <div className="text-2xl font-mono font-bold text-text">
                      ₱{(effectiveProject.totalProjectCost || 0).toLocaleString()}
                    </div>
                  </div>
                ...
                <div className="divide-y divide-border/50 text-xs">
                  <div className="py-2.5 flex items-center justify-between">
                    ...
                    <span className="font-mono font-bold text-text">
                      ₱{(effectiveProject.consumablesValue || 0).toLocaleString()}
                    </span>
                  </div>
              ...
              <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-border">
                ...
                <p className="text-xs text-text-secondary max-w-sm mt-1">
                  Your role does not permit access to direct budgetary figures and monetary audit logs.
                </p>
              </div>
            )}
          </section>
        ...
        <div className="flex shrink-0 items-center justify-between border-t border-border/80 bg-bg-subtle/50 px-5 py-3 text-xs text-text-secondary">
          <div className="flex items-center gap-2">
            <span className="font-mono font-bold text-text">{effectiveProject.projectCode}</span>
            <span>·</span>
