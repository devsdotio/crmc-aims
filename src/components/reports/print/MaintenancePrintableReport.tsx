...
export function MaintenancePrintableReport({
  ...
  const totalSpend = summary?.totalRepairSpend || data.reduce((sum, d) => sum + (d.repairCost || 0), 0);
  ...
  const controlNumber = `CRMC-CUST-MNT-${generatedAt.getFullYear()}${String(generatedAt.getMonth() + 1).padStart(2, "0")}-${String(generatedAt.getDate()).padStart(2, "0")}`;
    ...
    <div className="print-page mx-auto w-full bg-white text-black text-[9.5px] leading-tight font-sans space-y-2 p-0">
        ...
        <div className="mt-2 pt-1.5 border-t border-black flex items-end justify-between text-black">
          <div>
            <h1 className="text-sm font-black tracking-tight uppercase text-black">
              Custodian Report on Equipment Maintenance, Repair &amp; Work Orders
            </h1>
          ...
          <div className="text-right text-[9px] font-mono text-black">
            <span>CHED / COA Maintenance Accountability Standard</span>
          </div>
        </div>
      </header>
      ...
      <section className="avoid-break grid grid-cols-5 gap-2 text-center text-[10px] text-black">
        <div className="border border-black p-1.5 bg-white">
          ...
          <span className="text-sm font-black font-mono">{totalOrders}</span>
        </div>
        <div className="border border-black p-1.5 bg-white">
          <span className="text-[8.5px] font-bold uppercase tracking-wider block">
            Open / In Progress
          </span>
          <span className="text-sm font-black font-mono">{openOrders}</span>
        </div>
        <div className="border border-black p-1.5 bg-white">
          <span className="text-[8.5px] font-bold uppercase tracking-wider block">
            Resolved / Repaired
          </span>
          <span className="text-sm font-black font-mono">{resolvedOrders}</span>
        </div>
        <div className="border border-black p-1.5 bg-white">
          <span className="text-[8.5px] font-bold uppercase tracking-wider block">
            Mean Time to Repair
          </span>
          <span className="text-sm font-black font-mono">{avgMttr} Days MTTR</span>
        </div>
        <div className="border border-black p-1.5 bg-white">
          <span className="text-[8.5px] font-bold uppercase tracking-wider block">
          ...
          <span className="text-sm font-black font-mono">
            {canViewCosts ? formatPhp(totalSpend) : `${resolutionRate}% resolved`}
          </span>
        </div>
      </section>

      {/* ─── 3. Standard CHED Maintenance Ledger Table (Pure Black Ink) ───── */}
          ...
          <table className="w-full border-collapse text-left text-[9px] text-black">
            <thead>
              <tr className="bg-white text-black border-b-2 border-black uppercase font-bold text-[8.5px] tracking-wider avoid-orphan-header">
                ...
                <th className="py-2 px-2 border-r border-black whitespace-nowrap">WO / Log Code</th>
                <th className="py-2 px-2 border-r border-black whitespace-nowrap">Property Tag No.</th>
                <th className="py-2 px-2 border-r border-black min-w-40">Equipment Description</th>
                <th className="py-2 px-2 border-r border-black whitespace-nowrap">Fault Condition</th>
                <th className="py-2 px-2 border-r border-black text-center whitespace-nowrap">Status</th>
                <th className="py-2 px-2 border-r border-black whitespace-nowrap">Date Logged</th>
                <th className="py-2 px-2 text-right border-r border-black whitespace-nowrap">Repair Cost</th>
                <th className="py-2 px-2 min-w-28">Action Remarks</th>
              </tr>
            </thead>
              ...
              {data.length === 0 && (
                <tr>
                  <td colSpan={9} className="py-6 text-center italic text-[10px]">
                    No equipment maintenance or repair records found matching specified filters.
                  </td>
                </tr>
              )}
            </tbody>
            ...
            {data.length > 0 && (
              <tfoot>
                <tr className="bg-white font-black border-t-2 border-b-2 border-black text-black text-[9.5px]">
                  <td colSpan={7} className="py-1.5 px-2 text-right uppercase tracking-wider border-r border-black">
                    Grand Total Repair Expenditures:
                  </td>
                  <td className="py-1.5 px-2 text-right font-mono border-r border-black">
                    {canViewCosts ? formatPhp(totalSpend) : "—"}
                  </td>
                  <td className="py-1.5 px-2 text-[8.5px] font-sans font-normal">
                    {resolvedOrders} of {totalOrders} work orders completed ({resolutionRate}% resolution).
                  </td>
                </tr>
              </tfoot>
            )}
      ...
      <footer className="avoid-break pt-3 border-t-2 border-black space-y-3 text-black">
        <div className="grid grid-cols-4 gap-4 text-center">
          ...
          <div className="space-y-1">
            ...
            <div className="border-b border-black pt-7 mb-1" />
            <div className="text-[10px] font-bold uppercase">
              Maintenance Supervisor / Inspection Chair
            </div>
            <div className="text-[8.5px]">
        ...
        <div className="mt-2 pt-1.5 border-t border-black flex items-center justify-between text-[8px] font-mono text-black">
          ...
          <div>Official CHED Maintenance Summary Form · Pure Black Ink Folio</div>
