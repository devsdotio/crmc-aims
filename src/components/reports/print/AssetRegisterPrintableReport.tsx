      ...
      <section className="avoid-break mb-2.5">
        <div className="mb-1 flex items-center justify-between">
          <h3 className="text-[9.5px] font-bold uppercase tracking-wider text-teal-800">
            Category &amp; Equipment Classification Summary
          </h3>
          ...
          <table className="w-full border-collapse text-xs">
            <thead>
              <tr className="border-b border-neutral-200 bg-neutral-50 text-[8.5px] font-bold uppercase tracking-wider text-neutral-600">
                <th className="py-1 px-2 text-left">Category / Equipment Group</th>
                <th className="py-1 px-2 text-right">Total Units</th>
                <th className="py-1 px-2 text-left">Operational Readiness</th>
                <th className="py-1 px-2 text-left">Maintenance Status</th>
                <th className="py-1 px-2 text-left">Primary Department(s)</th>
                {canViewCosts && <th className="py-1 px-2 text-right">Group Valuation</th>}
              </tr>
            </thead>
              ...
              {categoryGroups.map((group) => {
                ...
                return (
                  <tr key={group.category}>
                    <td className="py-1 px-2 font-bold text-[#2A3260]">
                      <div>{group.category}</div>
                      {group.sampleNames.length > 0 && (
                        <div className="text-[8px] font-normal text-neutral-500 truncate max-w-44">
                          e.g. {group.sampleNames.join(", ")}
                        </div>
                      )}
                    </td>
                    <td className="py-1 px-2 text-right font-mono font-extrabold text-neutral-900">
                      {group.totalCount.toLocaleString()}{" "}
                      <span className="text-[8px] font-normal text-neutral-500">units</span>
                    </td>
                    ...
                    <td className="py-1 px-2 text-neutral-600 text-[8.5px] truncate max-w-40">
                      {topDepts || "Unassigned"}
                    </td>
                    {canViewCosts && (
